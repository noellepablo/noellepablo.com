...
Working at [ATLAS](https://atlas.ku.edu/) involves a lot of student assessment data, including text responses. For certain projects, the data is sent to our team with residual HTML code in the text response strings, for example, `It's` might appear as `It&#39;s`, since the HTML code for an apostrophe is denoted by `&#39;`. To make these responses more readable, it is helpful to clean, or replace, the HTML markup with the equivalent symbols.

The response data I most recently cleaned contained HTML code and various Unicode numbers, such as `U+200B`, which represents a zero width space. To clean the HTML code and Unicode numbers from the response strings, I used a combination of the `replace_html()` function within the `textclean` package and the `HTMLdecode()` function within the `textutils` package. Both functions were needed since there are some Unicode numbers that `replace_html()` recognizes that `HTMLdecode()` does not, and there are some HTML symbols that `HTMLdecode()` recognizes that `replace_html()` does not.
...
fix_html <- function(file){
  ...
  newfilename <- str_replace(file, ".csv", "_noCode.xlsx")
  data %>% 
